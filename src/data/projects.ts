...
export const projects: Project[] = [
  ...
  },
  {
    id: 'vivid',
    name: 'Vivid',
    tagline: 'Cinematic storyboard AI with identity locking and cost-aware GPU deployment.',
    status: 'complete',
    stack: [
      'FLUX.1-dev',
      'PuLID',
    ...
    panels: {
      problem: {
        quote: 'Creative generation is not useful if identity, cost, and shot intent drift apart.',
        brokenFlowId: 'storyboard-identity-drift',
      },
      architecture: {
        summary: 'A 4-shot storyboard pipeline plans, routes, locks identity, grades, scores, retries, and exports.',
        nodes: [
          {
            id: 'planner',
            label: 'Scene Planner',
            description: 'Groq plans shots from scene descriptions or screenplay text.',
            stackChips: ['Groq'],
            connections: ['router'],
            position: { x: 12, y: 34 },
          },
          {
            id: 'router',
            label: 'Shot Router',
            description: 'Routes human and non-human shots to the correct generation path.',
            stackChips: ['FastAPI'],
          ...
          },
          {
            ...
            description: 'FLUX.1-dev, PuLID, and LoRA style controls generate identity-aware frames.',
          ...
          {
            id: 'quality',
            label: 'Score And Export',
            description: 'CLIP scoring, auto-retry, color grading, and PDF export close the loop.',
            stackChips: ['CLIP scoring', 'PIL', 'PDF'],
            connections: [],
            position: { x: 86, y: 52 },
          },
        ],
      ...
      proof: {
        metrics: [
          ...
          { label: 'LoRA styles', value: 13, display: '13' },
          { label: 'Warm start', value: 5, display: '~5s', unit: 's' },
        ],
        radialMetricId: 'vivid-cost',
        caveat: 'Identity consistency is tuned and measured, not claimed as perfect.',
      },
      boundary: {
        items: [
          { side: 'will', text: 'Generate 4-shot cinematic storyboards from scene intent.' },
          { side: 'will', text: 'Tune PuLID and guidance to protect face identity.' },
          { side: 'will', text: 'Track GPU economics across Modal and RunPod paths.' },
          { side: 'refuses', text: 'Claim foundation-model training from scratch.' },
          { side: 'refuses', text: 'Expose API URLs, HF tokens, Groq keys, or deployment secrets.' },
        ],
      },
    },
    node: {
