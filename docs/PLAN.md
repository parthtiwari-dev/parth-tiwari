...
| ✅ 8.19 | **Hover was aimed at the wrong thing.** The raycast picks the star sphere and nothing else, and at overview distances that sphere is a few pixels which orbits out from under a *motionless* cursor in **3997ms** — measured on the deployed site, pointer held perfectly still. The name printed beside it, which is what a person actually aims at, did nothing. `pointer-events: auto` on the name is the wrong fix: the canvas owns drag-to-orbit and a press starting on a DOM child above it never reaches the canvas (8.16), so five interactive names would be five dead zones for the gesture that unlocks free mode. The hit test is geometric instead — pointer position against the boxes the labels were last drawn at, read at the top of the tick before that frame writes any transforms, so it is a clean read rather than a forced reflow. The same test drives the freeze, so a label the pointer is resting inside stops tracking its star and the target cannot slide away mid-reach. Measured after: card visible **100% of a 10s hold with zero flips**, at two aim points on two different projects, and 9s under a real motionless mouse. Note the sandbox renders at ~2.5fps, where labels sweep tens of pixels between ticks and acquisition needs a corrective move; that is a fact about the runner, not the site. |
| ✅ 8.20 | **The cursor kept naming something that was gone.** Cursor state is only ever recomputed from a `pointermove` target, so opening a project — a click, with the pointer perfectly still — left it reading ENTER over the panel's body copy, and it was *still* reading ENTER after the panel closed and the star had gone. Visible in the 8.18 capture at top-right. Re-resolving with `elementFromPoint` was tried first and reports the old DOM twice over: the watcher is pre-flush, and the overlay leaves through a `<Transition>`, so its close button is still under the pointer well after the overlay is logically gone — verified, it returned `enter` every time. It resets to `default` instead. A cursor that makes no claim until the pointer moves is correct; one that confidently names something no longer there is not. |
| ✅ 8.21 | **A full blank screen mid-scroll on mobile.** `.mobile-systems-index` carried `padding-top: 148vh` and `min-height: 220vh` — scroll runway for `MobileStarWorld`, a sticky canvas deleted in 2.5. Nothing has been sticky there since, so it rendered as **1249px of black** at 844px of viewport between the end of the contact panel and the first word of the systems index, on a document that is only 13 screens long. Measured gap section-top to first word: **1249px → 56px**, document height 10948 → 9755. |
| ✅ 8.22 | **QueryPilot's live API link hung.** `querypilot-backend.onrender.com` is a Render free-tier instance that sleeps; a warm-up request held the connection open for **90 seconds without answering** — not a slow page, a link that hangs, and the first thing a sceptical reader clicks. `liveAPI` and `apiDocs` removed; the repo stays. The panel captures are the served contract and stand on their own. `evidenceOf()` is unaffected because `hasPublicLink` still sees `github`, so the star does not change size. Same rule as the one that keeps unverified deployments out: an endpoint that may or may not answer is worse evidence than no endpoint. (The open-items list attributed this to `support-core`; that project's frontend returns 200 and is fine.) |
| ✅ 8.23 | **The loading screen measured nothing.** `useBootSequence` printed three string literals — `'> initializing evidence field...    ████████░░  80%'`, always eighty percent — and dismissed on a 2200ms timer, in front of a site whose stated rule is that no visual may claim something it does not encode. A progress bar is the purest form of that failure: it is *only* a claim about progress. `data/loadSignals.ts` now holds the real dependencies of a first frame — the code-split scene chunk and `document.fonts.ready` — and each bar flips when its promise settles. A task is registered only when it will genuinely happen: under reduced motion and in plain mode the scene chunk is never fetched, so declaring it would hang the boot on a promise nobody created. Verified throttled to 400kbps: typefaces reach `ready` while the scene bundle is still `loading`, which is a thing the old screen could not have shown. Two bounds remain and neither pretends to measure: `MIN_VISIBLE_MS` (450) stops a warm cache flashing the terminal for one frame, and `MAX_WAIT_MS` (**3500, not 6000**) stops an honest wait becoming a worse one — unthrottled it left at 3566ms, at 400kbps the 6000ms cap let it hold for **10130ms** against the 2200ms the fake timer took. The hero underneath is DOM and needs no bundle to be worth reading. |
| ✅ 8.24 | **The twelve projects were the fifteenth thing a keyboard reached.** Tab order follows the DOM and `ProjectIndex` was declared next to last, so a keyboard user passed the zoom controls, the booking link, both contact fields, the email and WhatsApp links and all five evidence buttons before arriving at the rail that opens the work the site is about. Declaration moved to the top of the template; it costs nothing visually because the rail is `position: fixed` with an explicit `z-index`, so paint order does not depend on where it sits. Measured at 1440: **stop 15 → stop 1**. |
| ✅ 8.25 | **Android Back ejected the visitor instead of closing the panel.** 4.7 chose `replaceState` on the argument that opening a panel is not navigation, so Back should leave rather than dismiss. Tested on the phone profile this site is built for, the argument inverts: traffic arrives from a DM link on Android, where the system Back gesture *is* how anything covering the screen gets dismissed. Measured on the deployed site — open a project, press Back, `about:blank`. Trapping someone was the fear; ejecting them was the behaviour. `pushState` on the first open, `replace` when switching projects with the panel already up, and a close we initiated calls `history.back()` so Escape and the X leave the stack exactly as they found it. Arriving directly on `?project=x` never calls `back()` — there is no entry of ours to pop and the previous page belongs to whoever linked here. Verified: open → Back closes the panel and stays on the site; open → close via X → Back leaves the site with no dead entry. |
| 🟡 8.26 | **"Open on evidence, not a quote" was already built; the gap is data.** `hasShowcase()` in `data/showcase.ts` already puts a Demo panel first wherever a project has `images`, `video` or `outcome`, and `panelCountFor` is its single owner. Audited all twelve: **five open on a visual** (beatmind, stick-and-dot, querypilot, support-core, tathya) and **seven still open on a pull-quote** because they genuinely have no capture and no outcome line — secondself, upi-fraud, medrag, oncoverse, order-supervisor, fraud-risk-intel, oracle-auto-provision. No code change would fix that honestly; an empty showcase is the decoration-pretending-to-be-evidence this codebase exists to avoid. Left open: it needs `npm run capture` against confirmed-public deployments, or real outcome lines, and only three projects carry an `outcome` today. |
